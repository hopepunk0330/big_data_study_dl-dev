...
- 머신러닝은 **아나콘다(conda)**, 딥러닝은 venv가 낫다는 게 개인 판단(노트에 명시).
  - **⚠️ 확인 필요**: 이 판단은 이후 바뀌었다. 딥러닝 저장소 `dl-dev`는 실제로 **conda 환경(`dl-dev`)**을 쓴다(2026-09-01 확인). **"DL이니까 venv"라고 추론하지 말고, 그 프로젝트 `CLAUDE.md`의 개발 환경 절과 `ml-project-workflow.md` 0절의 탐지 절차로 실제 상태를 확인한다.** 노트 원문은 기록으로 그대로 남긴다.
