...
- dl-dev는 딥러닝 교재(chap01~10) 학습 저장소 — `CLAUDE.md`는 2절 절차대로 개요·폴더 구조·개발 환경(conda 환경 `dl-dev`/Python 3.11/PyTorch — 최초엔 venv로 잘못 기록했다가 2026-09-01 같은 날 정정)·검증 명령·금지 항목을 이 프로젝트에 맞게 수정했고, 행동 지침은 이식된 규칙을 그대로 유지했다(현재 워크스트림에서 안 쓰는 규칙도 삭제하지 않음).
